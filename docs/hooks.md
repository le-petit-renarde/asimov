...
Hooks let you run arbitrary shell commands (or HTTP requests, LLM prompts, or agentic verifiers) in response to events that happen inside a Asimov session. They are the primary mechanism for extending and automating Asimov's behavior without modifying the agent itself.

---

## What hooks are

A hook is a piece of executable logic that Asimov calls at a specific lifecycle event. The simplest kind is a shell command. When the event fires, Asimov:
...
Asimov supports four hook implementations:
...
{
  "type": "http",
  "url": "https://hooks.example.com/asimov",
  "headers": {
...
Fires when Asimov sends a notification. The matcher field is compared against `notification_type`.
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "python3 ~/.asimov/hooks/check_bash.py"
          }
        ]
      }
    ],
    ...
    "SessionEnd": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "bash ~/.asimov/hooks/notify-slack.sh"
          }
        ]
      }
    ]
    ...
    "PreToolUse": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "jq -c '{ts: now | todate, event: .hook_event_name, tool: .tool_name, input: .tool_input}' >> ~/.asimov/tool.log"
          }
        ]
      }
    ]
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash ~/.asimov/hooks/guard.sh"
          }
        ]
      }
    ]
...
```bash
#!/usr/bin/env bash
# ~/.asimov/hooks/slack-session-end.sh
INPUT=$(cat)
REASON=$(echo "$INPUT" | jq -r '.reason')

curl -s -X POST "$SLACK_WEBHOOK_URL" \
  -H 'Content-Type: application/json' \
  -d "{\"text\": \"Asimov session ended (reason: ${REASON})\"}"
```
    ...
    "SessionEnd": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "bash ~/.asimov/hooks/slack-session-end.sh"
          }
        ]
      }
    ]
