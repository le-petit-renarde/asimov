...
The Model Context Protocol is a JSON-RPC 2.0 based protocol for connecting Asimov to external tool, resource, and prompt servers. MCP servers extend what the agent can do without modifying Asimov itself — they can expose file systems, databases, APIs, browser automation, and anything else that can be wrapped in a tool.
...
Asimov discovers tools, resources, and prompts from connected MCP servers during the handshake phase and wraps them as native `Tool` instances (via `McpToolWrapper`), making them transparent to the query loop.
...
The default transport. Asimov spawns the server as a child process and communicates over its stdin/stdout using newline-delimited JSON-RPC 2.0.
...
Add servers to the `config.mcp_servers` array in `~/.asimov/settings.json`:
...
Project-level MCP servers can be added to a `.asimov/settings.json` in your project root. Project settings take precedence over global settings.
...
When an MCP server disconnects or fails to connect, Asimov starts a background reconnection loop automatically:
