...
/// Scan `project_root` for skill definitions in `.asimov/skills/` and the bundled
...
pub async fn prefetch_skills(project_root: &Path, index: SharedSkillIndex) {
    ...
    // 1. User-defined skills: ~/.asimov/skills/*.md + {project_root}/.asimov/skills/*.md
    let search_dirs: Vec<std::path::PathBuf> = {
        let mut dirs = Vec::new();
        if let Some(home) = dirs::home_dir() {
            dirs.push(home.join(".asimov").join("skills"));
        }
        dirs.push(project_root.join(".asimov").join("skills"));
        dirs
    };
