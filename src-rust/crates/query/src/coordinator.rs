...
pub const COORDINATOR_ENV_VAR: &str = "ASIMOV_COORDINATOR_MODE";
...
/// Tools that workers are allowed to use in simple mode (ASIMOV_SIMPLE=1).
...
pub fn filter_tools_for_mode<'a>(
    tools: &'a [Box<dyn asimov_tools::Tool>],
    mode: AgentMode,
) -> Vec<&'a Box<dyn asimov_tools::Tool>> {
    match mode {
