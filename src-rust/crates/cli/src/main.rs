// asimov CLI entry point
//
// This is the main binary for Asimov. It:
...
struct McpToolWrapper {
    ...
    server_name: String,
    manager: Arc<asimov_mcp::McpManager>,
}
...
impl Tool for McpToolWrapper {
    ...
    async fn execute(&self, input: serde_json::Value, ctx: &ToolContext) -> ToolResult {
        ...
        match self.manager.call_tool(&self.tool_def.name, args).await {
            Ok(result) => {
                let text = asimov_mcp::mcp_result_to_string(&result);
                if result.is_error {
...
#[command(
    name = "claude",
    version = APP_VERSION,
    about = "Asimov - AI-powered coding assistant",
    long_about = None,
)]
struct Cli {
    ...
    /// Grant Asimov access to an additional directory (can be repeated)
    ...
    #[arg(long, env = "ASIMOV_PROVIDER")]
    provider: Option<String>,

    /// Override the API base URL for the selected provider
    #[arg(long, env = "ASIMOV_API_BASE")]
...
impl From<CliOutputFormat> for asimov_core::config::OutputFormat {
    fn from(f: CliOutputFormat) -> Self {
        match f {
            CliOutputFormat::Text => asimov_core::config::OutputFormat::Text,
            CliOutputFormat::Json => asimov_core::config::OutputFormat::Json,
            CliOutputFormat::StreamJson => asimov_core::config::OutputFormat::StreamJson,
        }
    }
}
...
fn resolve_bridge_config(
    ...
    is_headless: bool,
) -> Option<asimov_bridge::BridgeConfig> {
    if is_headless {
        return None;
    }

    let mut bridge_config = asimov_bridge::BridgeConfig::from_env();
...
async fn main() -> anyhow::Result<()> {
    ...
    // Fast-path: `asimov upgrade [--version <v>] [--force]` — self-update.
    ...
    if raw_args.get(1).map(|s| s.as_str()) == Some("acp") {
        return asimov_acp::run_acp_server().await;
    }

    // Fast-path: `asimov models [provider] [--refresh] [--verbose] [--json]`
    ...
    if let Some(cmd_name) = raw_args.get(1).map(|s| s.as_str()) {
        ...
        if !cmd_name.starts_with('-') && !cmd_name.starts_with('/') {
            if let Some(named_cmd) = asimov_commands::named_commands::find_named_command(cmd_name) {
                ...
                let cmd_ctx = asimov_commands::CommandContext {
                    config,
                ...
                match result {
                    asimov_commands::CommandResult::Message(msg)
                    | asimov_commands::CommandResult::UserMessage(msg) => {
                        println!("{}", msg);
                        std::process::exit(0);
                    }
                    asimov_commands::CommandResult::Error(e) => {
                        eprintln!("Error: {}", e);
    ...
    debug!(cwd = %cwd.display(), "Starting Asimov");
                ...
                if is_headless {
                    anyhow::bail!(
                        "No API key found. Options:\n\
                         ...
                         - Run `asimov --provider ollama` for local models (no key needed)\n\
                         - Run `asimov auth login` for Anthropic OAuth"
                    );
                } else {
                    (String::new(), false)
                }
    ...
    let client = Arc::new(
        asimov_api::AnthropicClient::new(client_config.clone())
            .context("Failed to create API client")?,
    );

    // Build provider registry: auto-registers all env-configured providers
    // AND providers with keys stored in ~/.asimov/auth.json (from /connect).
    ...
    let provider_registry = asimov_api::ProviderRegistry::from_config(&config, client_config);
    ...
    let permission_handler: Arc<dyn asimov_core::PermissionHandler> = if is_headless {
    ...
    let pending_permissions = Arc::new(ParkingMutex::new(asimov_tools::PendingPermissionStore::default()));
    ...
    let (user_question_tx, user_question_rx) =
        tokio::sync::mpsc::unbounded_channel::<asimov_tools::UserQuestionEvent>();
    ...
    if config.auto_commits == Some(true) {
        ...
        tokio::spawn(async move {
            ...
            loop {
                if let Some(snap) = asimov_core::snapshot::get_or_create(&gc_dir) {
                    snap.cleanup().await;
                }
    ...
    // but we guard with a std::sync::OnceLock internally).
    {
        ...
        SWARM_INIT.get_or_init(|| asimov_query::init_team_swarm_runner());
    }
    ...
    let plugin_registry = asimov_plugins::load_plugins(&cwd, &[]).await;
    {
    ...
    // Build query config
    let mut query_config = asimov_query::QueryConfig::from_config_with_registry(&config, &model_registry);
    ...
    if let Some(ref level_str) = cli.effort {
        if let Some(level) = asimov_core::effort::EffortLevel::from_str(level_str) {
    ...
    let tools = if let Some(ref agent_name) = cli.agent {
        ...
        let mut all_agents = asimov_core::default_agents();
    ...
    asimov_query::start_cron_scheduler(
        client.clone(),
        tools.clone(),
        tool_ctx.clone(),
        query_config.clone(),
    ...
    let result = if is_headless {
        ...
        let has_saved_credentials = !auth_store.credentials.is_empty()
            || asimov_core::oauth_config::get_codex_tokens().is_some();
...
async fn connect_mcp_manager_arc(
    config: &Config,
) -> Option<Arc<asimov_mcp::McpManager>> {
    if config.mcp_servers.is_empty() {
        return None;
    }

    info!(count = config.mcp_servers.len(), "Connecting to MCP servers");
    let mcp_manager = Arc::new(asimov_mcp::McpManager::connect_all(&config.mcp_servers).await);
    mcp_manager.clone().spawn_notification_poll_loop();
    Some(mcp_manager)
}

fn build_tools_with_mcp(
    mcp_manager: Option<Arc<asimov_mcp::McpManager>>,
) -> Arc<Vec<Box<dyn asimov_tools::Tool>>> {
    let mut v: Vec<Box<dyn asimov_tools::Tool>> = asimov_tools::all_tools();
    v.push(Box::new(asimov_query::AgentTool));
...
/// `ASIMOV_MODELS_URL` doesn't stomp the canonical models.dev cache.
...
/// Implementation of the `asimov models` subcommand.
///
/// Flags:
...
async fn run_models_command(args: &[String]) -> anyhow::Result<()> {
    ...
    for arg in args {
        match arg.as_str() {
            ...
            "--json" => as_json = true,
            s if s.starts_with("--") => {
                eprintln!("asimov models: unknown flag: {}", s);
                eprintln!("Usage: asimov models [<provider>] [--refresh] [--verbose] [--json]");
                std::process::exit(2);
            }
            s => {
                if provider_filter.is_some() {
                    eprintln!("asimov models: only one provider id may be supplied");
                    std::process::exit(2);
                }
    ...
    let mut registry = asimov_api::ModelRegistry::new()
    ...
    let mut entries: Vec<&asimov_api::ModelEntry> = match &provider_filter {
    ...
    if as_json {
        ...
        let mut map: std::collections::BTreeMap<String, &asimov_api::ModelEntry> =
    ...
    if entries.is_empty() {
        if let Some(pid) = &provider_filter {
            ...
            eprintln!("Try: asimov models                # list all providers");
            eprintln!("     asimov models --refresh      # pull latest from models.dev");
        } else {
            eprintln!("No models in registry.  Try `asimov models --refresh`.");
        }
        return Ok(());
    }
    ...
    for entry in &entries {
        ...
        if verbose {
            ...
            if !matches!(entry.status, asimov_api::ModelStatus::Active) {
    ...
    if provider_filter.is_none() {
        eprintln!(
            "\n{} models across {} providers.  Use `asimov models <provider>` to filter.",
            total,
...
fn load_cached_model_registry() -> Arc<asimov_api::ModelRegistry> {
    let mut reg = asimov_api::ModelRegistry::new();
    // ASIMOV_MODELS_PATH wins outright — useful for offline dev where you
    // pin a known-good api.json on disk.
    if let Ok(custom) = std::env::var("ASIMOV_MODELS_PATH") {
...
///
/// Honors:
/// * `ASIMOV_DISABLE_MODELS_FETCH` — skips the network call entirely.
/// * `ASIMOV_MODELS_URL` / `MODELS_DEV_URL` — overrides the source URL.
...
fn spawn_models_cache_refresh() {
    if std::env::var("ASIMOV_DISABLE_MODELS_FETCH").is_ok() {
        tracing::debug!("ASIMOV_DISABLE_MODELS_FETCH set — skipping models.dev refresh");
        return;
    }
    ...
    tokio::spawn(async move {
        ...
        let resp = match client
            .get(&url)
            .header("User-Agent", concat!("Asimov/", env!("CARGO_PKG_VERSION")))
            .send()
            .await
        {
            Ok(r) => r,
...
struct RefreshedProviderRuntime {
    config: Config,
    client: Arc<asimov_api::AnthropicClient>,
    provider_registry: Arc<asimov_api::ProviderRegistry>,
    model_registry: Arc<asimov_api::ModelRegistry>,
...
async fn refresh_provider_runtime_state(
    ...
    remove_file_if_exists(&asimov_core::AuthStore::path())
        .await
        .context("Failed to clear auth store")?;
    remove_file_if_exists(&asimov_core::oauth::OAuthTokens::token_file_path())
        .await
    ...
    let provider_registry =
        Arc::new(asimov_api::ProviderRegistry::from_config(&config, client_config));
    let model_registry = load_cached_model_registry();
    ...
    Ok(RefreshedProviderRuntime {
        ...
        provider_registry,
        model_registry,
        auth_store: asimov_core::AuthStore::default(),
    })
}
...
fn filter_tools_for_agent(
    tools: Arc<Vec<Box<dyn asimov_tools::Tool>>>,
    access: &str,
) -> Arc<Vec<Box<dyn asimov_tools::Tool>>> {
    use asimov_tools::PermissionLevel as PL;
    match access {
        "read-only" => {
            ...
            let allowed_names: Vec<String> = tools
                ...
                .collect();
            let filtered: Vec<Box<dyn asimov_tools::Tool>> = asimov_tools::all_tools()
                .into_iter()
...
async fn run_headless(
    ...
    query_config: asimov_query::QueryConfig,
    cost_tracker: Arc<CostTracker>,
) -> anyhow::Result<()> {
    use asimov_query::{QueryEvent, QueryOutcome};
    use tokio::sync::mpsc;
    ...
    let mut messages: Vec<asimov_core::types::Message> = if cli.input_format == CliInputFormat::StreamJson {
        use tokio::io::{self, AsyncBufReadExt, BufReader};
        let stdin = io::stdin();
        let mut reader = BufReader::new(stdin);
        let mut line = String::new();
        let mut parsed: Vec<asimov_core::types::Message> = Vec::new();
        loop {
            line.clear();
            ...
            match serde_json::from_str::<serde_json::Value>(trimmed) {
                Ok(v) => {
                    ...
                    let content = v
                        ...
                        .to_string();
                    if role == "assistant" {
                        parsed.push(asimov_core::types::Message::assistant(content));
                    } else {
                        parsed.push(asimov_core::types::Message::user(content));
                    }
                }
                Err(e) => {
        ...
        if parsed.is_empty() {
            ...
            if let Some(ref p) = cli.prompt {
                parsed.push(asimov_core::types::Message::user(p.clone()));
            }
        }
        parsed
    } else {
        ...
        vec![asimov_core::types::Message::user(prompt)]
    };
    ...
    if let Some(ref prefill_text) = cli.prefill {
        messages.push(asimov_core::types::Message::assistant(prefill_text.clone()));
    }
    ...
    while let Some(event) = event_rx.recv().await {
        match &event {
            QueryEvent::Stream(asimov_api::AnthropicStreamEvent::ContentBlockDelta {
                delta: asimov_api::streaming::ContentDelta::TextDelta { text },
                ..
            }) => {
    ...
    let outcome = query_handle.await.unwrap_or(QueryOutcome::Error(
        asimov_core::error::ClaudeError::Other("Query task panicked".to_string()),
    ));
...
fn permission_request_from_core(
    pending: &asimov_tools::PendingPermissionRequest,
) -> asimov_tui::dialogs::PermissionRequest {
    ...
    match (tool_name.as_str(), pending.request.path.clone()) {
        ("Bash", Some(command)) => {
            ...
            asimov_tui::dialogs::PermissionRequest::bash(
                tool_use_id,
                tool_name,
                reason,
                command,
        ...
        ("PowerShell", Some(command)) => asimov_tui::dialogs::PermissionRequest::powershell(
            tool_use_id,
            tool_name,
            reason,
            command,
        ),
        ("Read", Some(path)) => asimov_tui::dialogs::PermissionRequest::file_read(
            tool_use_id,
            tool_name,
            reason,
            path,
        ...
        (_, Some(path)) if matches!(tool_name.as_str(), "Write" | "Edit" | "NotebookEdit") => {
            asimov_tui::dialogs::PermissionRequest::file_write(tool_use_id, tool_name, reason, path)
        }
        _ => asimov_tui::dialogs::PermissionRequest::from_reason(
            tool_use_id,
            tool_name,
            reason,
...
async fn run_interactive(
    config: Config,
    settings: asimov_core::config::Settings,
    client: Arc<asimov_api::AnthropicClient>,
    tools: Arc<Vec<Box<dyn asimov_tools::Tool>>>,
    tool_ctx: ToolContext,
    query_config: asimov_query::QueryConfig,
    ...
    bridge_config: Option<asimov_bridge::BridgeConfig>,
    has_credentials: bool,
    model_registry: Arc<asimov_api::ModelRegistry>,
    user_question_rx: Option<tokio::sync::mpsc::UnboundedReceiver<asimov_tools::UserQuestionEvent>>,
) -> anyhow::Result<()> {
    use asimov_commands::{execute_command, CommandContext, CommandResult};
    use asimov_bridge::{BridgeOutbound, TuiBridgeEvent};
    use asimov_query::{QueryEvent, QueryOutcome};
    use asimov_tui::{
    ...
    let mut session = if let Some(ref id) = resume_id {
        match asimov_core::history::load_session(id).await {
            Ok(session) => {
            ...
            Err(e) => {
                ...
                let mut session =
                    asimov_core::history::ConversationSession::new(
                        asimov_api::effective_model_for_config(&config, &model_registry),
                    );
    ...
    let pending_permissions = tool_ctx
        .pending_permissions
        .clone()
        .unwrap_or_else(|| Arc::new(ParkingMutex::new(asimov_tools::PendingPermissionStore::default())));
    ...
    if let Some(level) = base_query_config.effort_level {
        use asimov_tui::EffortLevel as TuiEL;
        app.effort_level = match level {
            asimov_core::effort::EffortLevel::Low    => TuiEL::Low,
            asimov_core::effort::EffortLevel::Medium => TuiEL::Normal,
            asimov_core::effort::EffortLevel::High   => TuiEL::High,
            asimov_core::effort::EffortLevel::Max    => TuiEL::Max,
        };
    }
    ...
    if !has_credentials {
        ...
        let _ = asimov_tui::App::persist_onboarding_complete_pub();
    }
    ...
    {
        let current_version = asimov_core::constants::APP_VERSION.to_string();
        if settings.last_seen_version.as_deref() != Some(&current_version) {
            ...
            tokio::spawn(async move {
                if let Ok(mut s) = asimov_core::config::Settings::load().await {
    ...
    let mut bridge_runtime: Option<BridgeRuntime> = if let Some(cfg) = bridge_config {
        ...
        tokio::spawn(async move {
            if let Err(e) = asimov_bridge::run_bridge_loop(cfg, tui_tx, outbound_rx, cancel_clone).await {
    ...
    let mut bridge_session_info: Option<std::sync::Arc<asimov_bridge::BridgeSessionInfo>> = None;
    ...
    let all_tools_arc: Arc<Vec<Box<dyn asimov_tools::Tool>>> =
        Arc::new(asimov_tools::all_tools());
    let mut tools_arc = tools;

    // Current cancel token (replaced each turn)
    let mut cancel: Option<CancellationToken> = None;
    let (event_tx, mut event_rx) = mpsc::unbounded_channel::<QueryEvent>();
    type MessagesArc = Arc<tokio::sync::Mutex<Vec<asimov_core::types::Message>>>;
    ...
    let mut current_effort: Option<asimov_core::effort::EffortLevel> = None;
    ...
    tokio::spawn(async move {
        let info = asimov_core::check_for_updates().await;
    ...
    enum McpAuthEvent {
        ...
        Completed(asimov_mcp::oauth::McpAuthResult),
    ...
    let mcp_auth_runner: Arc<dyn Fn(asimov_mcp::oauth::McpAuthSession) + Send + Sync> = {
        let tx = mcp_auth_tx.clone();
        Arc::new(move |session| {
            let tx = tx.clone();
            tokio::spawn(async move {
                let event = match asimov_mcp::oauth::run_mcp_auth_session(session).await {
    ...
    'main: loop {
            ...
            match evt {
                Event::Key(key) => {
                    ...
                    if key.code == KeyCode::Enter && !app.is_streaming && !any_dialog_open {
                        ...
                        if input.starts_with('/') {
                            let (cmd_name, cmd_args) =
                                asimov_tui::input::parse_slash_command(&input);
                            ...
                            if handled_by_tui && cmd_name == "effort" && cmd_args.is_empty() {
                                current_effort = Some(match app.effort_level {
                                    asimov_tui::EffortLevel::Low =>
                                        asimov_core::effort::EffortLevel::Low,
                                    asimov_tui::EffortLevel::Normal =>
                                        asimov_core::effort::EffortLevel::Medium,
                                    asimov_tui::EffortLevel::High =>
                                        asimov_core::effort::EffortLevel::High,
                                    asimov_tui::EffortLevel::Max =>
                                        asimov_core::effort::EffortLevel::Max,
                                });
                            }
                            ...
                            match cli_result {
                                ...
                                Some(CommandResult::ResumeSession(resumed_session)) => {
                                    ...
                                    asimov_tui::update_terminal_title(
                                ...
                                Some(CommandResult::RenameSession(title)) => {
                                    ...
                                    cmd_ctx.session_title = session.title.clone();
                                    let _ =
                                        asimov_core::history::save_session(&session).await;
                                    asimov_tui::update_terminal_title(Some(&title));
                                    app.status_message = Some(format!(
                                        "Session renamed to \"{}\".",
                                        title
                                    ));
                                        ...
                                        match refresh_provider_runtime_state(&cmd_ctx.config).await {
                                            Ok(refreshed) => {
                                                ...
                                                base_query_config.model =
                                                    asimov_api::effective_model_for_config(
                                                        &cmd_ctx.config,
                                ...
                                Some(CommandResult::Message(msg)) => {
                                    ...
                                    if !handled_by_tui {
                                        app.push_message(
                                            asimov_core::types::Message::assistant(msg),
                                        );
                                    }
                                }
                                Some(CommandResult::ConfigChange(new_cfg)) => {
                                    ...
                                    session.model = asimov_api::effective_model_for_config(
                                        &cmd_ctx.config,
                                        &model_registry,
                                    );
                                    app.status_message =
                                ...
                                Some(CommandResult::StartOAuthFlow(with_claude_ai)) => {
                                    asimov_tui::restore_terminal(&mut terminal).ok();
                                    ...
                                    terminal = asimov_tui::setup_terminal()?;
                                }
                            ...
                            {
                                if let Some(level) =
                                    asimov_core::effort::EffortLevel::from_str(&cmd_args)
                                {
                            ...
                            if let Some(msg) = submit_user_msg {
                                messages.push(asimov_core::types::Message::user(msg.clone()));
                                app.push_message(asimov_core::types::Message::user(msg));
                        ...
                        if !config.hooks.is_empty() {
                            let hook_ctx = asimov_core::hooks::HookContext {
                            ...
                            asimov_core::hooks::run_hooks(
                                &config.hooks,
                                asimov_core::config::HookEvent::UserPromptSubmit,
                                &hook_ctx,
                        ...
                        let user_msg = if pending_imgs.is_empty() {
                            asimov_core::types::Message::user(input.clone())
                        } else {
                            let mut blocks: Vec<asimov_core::types::ContentBlock> = pending_imgs
                                .iter()
                                .filter_map(|img| {
                                    asimov_tui::image_paste::encode_image_base64(&img.path)
                                        .map(|b64| asimov_core::types::ContentBlock::Image {
                                            source: asimov_core::types::ImageSource {
                                ...
                                .collect();
                            blocks.push(asimov_core::types::ContentBlock::Text { text: input.clone() });
                            asimov_core::types::Message::user_blocks(blocks)
                        };
                        ...
                        if session.title.is_some() {
                            asimov_tui::update_terminal_title(session.title.as_deref());
                        } else {
                            // Use a truncated version of the first user message
                            let topic: String = input.chars().take(60).collect();
                            asimov_tui::update_terminal_title(Some(&topic));
                        }
                        ...
                        qcfg.model = asimov_api::effective_model_for_config(&cmd_ctx.config, &model_registry);
                        ...
                        if let Some(goal) = asimov_core::GoalStore::open_default()
                            .and_then(|s| s.get_active_goal(&session.id))
                        {
                            let addendum = asimov_core::goal_system_prompt_addendum(&goal);
                        ...
                        if let Some(ref cq) = qcfg.command_queue {
                            let cq = cq.clone();
                            ctx_clone.completion_notifier = Some(asimov_tools::CompletionNotifier::new(move |msg| {
                                cq.push(
                                    asimov_query::QueuedCommand::InjectSystemMessage(msg),
                                    asimov_query::CommandPriority::Normal,
                                );
                            }));
                        }
                        ...
                        let handle = tokio::spawn(async move {
                            ...
                            let outcome = asimov_query::run_query_loop(
                                client_clone.as_ref(),
                                &mut msgs,
                    ...
                    if let Some(pr) = app.permission_request.as_mut() {
                        if asimov_tui::dialogs::handle_permission_key(pr, key) {
                            ...
                            let bash_prefix = if should_record_bash_prefix {
                                match &pr.kind {
                                    asimov_tui::dialogs::PermissionDialogKind::Bash { command, .. } => {
                            ...
                            if let Some(mut pending) = pending_permissions.lock().waiting.remove(&tool_use_id) {
                                let decision = match selected_key {
                                    Some('n') => asimov_core::permissions::PermissionDecision::Deny,
                                    _ => asimov_core::permissions::PermissionDecision::Allow,
                                };
                                ...
                                if let Some(manager) = tool_ctx.permission_manager.as_ref() {
                                    if let Ok(mut manager) = manager.lock() {
                                        match selected_key {
                                            ...
                                            Some('p') => {
                                                let mut settings = match asimov_core::config::Settings::load_sync() {
                                                    Ok(s) => s,
                                                    Err(_) => asimov_core::config::Settings::default(),
                                                };
                                                if let Some(path) = selected_path.as_deref() {
        ...
        if app.permission_request.is_none() {
            loop {
                ...
                let reevaluated = if prefix_allowed {
                    Some(asimov_core::permissions::PermissionDecision::Allow)
                } else {
                    tool_ctx
                        .permission_manager
                        .as_ref()
                ...
                match reevaluated {
                    Some(asimov_core::permissions::PermissionDecision::Ask { .. }) | None => {
                        let tool_use_id = pending.tool_use_id.clone();
            ...
            && !app.auto_compact_running
        {
            ...
            if used_pct >= 99 {
                ...
                let user_msg = asimov_core::types::Message::user(compact_msg);
                messages.push(user_msg.clone());
                app.push_message(user_msg);
                session.messages = messages.clone();
                session.updated_at = chrono::Utc::now();
            ...
            loop {
                match runtime.tui_rx.try_recv() {
                    Ok(TuiBridgeEvent::Connected { session_url, session_id: conn_sid }) => {
                        ...
                        session.updated_at = chrono::Utc::now();
                        let _ = asimov_core::history::save_session(&session).await;
                        ...
                        if let Some(ref token) = bridge_token {
                            let info = std::sync::Arc::new(asimov_bridge::BridgeSessionInfo {
                            ...
                            if let Some(rx) = relay_ev_rx_opt.take() {
                                ...
                                tokio::spawn(async move {
                                    ...
                                    while let Some(payload) = rx.recv().await {
                                        let _ = asimov_bridge::post_bridge_event(
                                            &info_relay,
                                            payload,
                                        )
                                        .await;
                            ...
                            tokio::spawn(async move {
                                ...
                                loop {
                                    match asimov_bridge::poll_bridge_messages(
                                        &info_poll,
                                        since_id.as_deref(),
                                    )
                                    .await
                    ...
                    Ok(TuiBridgeEvent::InboundPrompt { content, .. }) => {
                        ...
                        messages.push(asimov_core::types::Message::user(content.clone()));
                        app.push_message(asimov_core::types::Message::user(content.clone()));
                        session.messages = messages.clone();
                        session.updated_at = chrono::Utc::now();
                        app.is_streaming = true;
                        app.streaming_text.clear();
                    ...
                    Ok(TuiBridgeEvent::PermissionResponse { tool_use_id, response }) => {
                        ...
                        if let Some(ref pr) = app.permission_request {
                            if pr.tool_use_id == tool_use_id {
                                use asimov_bridge::PermissionResponseKind;
                                let _allow = matches!(
                                    response,
        ...
        if app.model_picker_fetch_pending {
            ...
            if let Some(ref registry) = app.provider_registry {
                let pid = asimov_core::ProviderId::new(&provider_id_str);
                if let Some(provider) = registry.get(&pid) {
                    ...
                    tokio::spawn(async move {
                        match provider.list_models().await {
                            Ok(models) => {
                                let entries: Vec<asimov_tui::model_picker::ModelEntry> = models
                                    .into_iter()
                                    .map(|m| asimov_tui::model_picker::ModelEntry {
                                        id: m.id.to_string(),
                                        display_name: m.name.clone(),
                                        description: asimov_tui::model_picker::format_context_window(
                                            m.context_window,
                                        ),
                                        is_current: false,
                                    })
        ...
        if app.tasks_overlay.visible {
            app.tasks_overlay.refresh_tasks(&asimov_tools::TASK_STORE);
        }
            ...
            match provider_id.as_str() {
                "github-copilot" => {
                    ...
                    tokio::spawn(async move {
                        ...
                        match asimov_core::device_code::request_device_code(
                            COPILOT_CLIENT_ID,
                            "read:user",
                            ...
                            Ok(resp) => {
                                ...
                                match asimov_core::device_code::poll_for_token(
                                    COPILOT_CLIENT_ID,
                                    &resp.device_code,
                ...
                "anthropic" => {
                    ...
                    // Asimov does not have its own registered OAuth app with Anthropic.
        ...
        while let Ok(evt) = device_auth_rx.try_recv() {
            match evt {
                DeviceAuthEvent::GotCode {
                    ...
                    let _ = asimov_tui::try_copy_to_clipboard(&user_code);
                    ...
                    app.notifications.push(
                        asimov_tui::NotificationKind::Info,
                ...
                DeviceAuthEvent::GotBrowserUrl { url } => {
                    ...
                    let _ = asimov_tui::try_copy_to_clipboard(&url);
        ...
        if task_finished {
            if let Some((handle, msgs_arc)) = current_query.take() {
                ...
                session.messages = messages.clone();
                session.updated_at = chrono::Utc::now();
                session.model = asimov_api::effective_model_for_config(&cmd_ctx.config, &model_registry);
                session.working_dir = Some(tool_ctx.working_dir.display().to_string());
                app.is_streaming = false;
                ...
                // Also index into SQLite for /search support
                {
                    ...
                    if let Ok(store) = asimov_core::SqliteSessionStore::open(&db_path) {
                        ...
                        for msg in &session.messages {
                            let content_str = match &msg.content {
                                asimov_core::types::MessageContent::Text(t) => t.clone(),
                                asimov_core::types::MessageContent::Blocks(blocks) => blocks.iter()
                                    .filter_map(|b| if let asimov_core::types::ContentBlock::Text { text } = b { Some(text.as_str()) } else { None })
                                    .collect::<Vec<_>>()
                                    .join(" "),
                            };
                ...
                if !app.auto_compact_running && asimov_core::goals_enabled() {
                    let elapsed_secs = goal_turn_start.elapsed().as_secs();
                    let total_tokens = cost_tracker.total_tokens();
                    match asimov_query::check_and_continue_goal(
                        &session.id,
                        total_tokens,
                        elapsed_secs,
                    ) {
                        asimov_query::GoalContinuation::Continue { message } => {
                            ...
                            let cont_msg = asimov_core::types::Message::user(message);
                        ...
                        asimov_query::GoalContinuation::Stop { reason } => {
                            app.active_goal_badge = None;
                            if let Some(msg) = reason.user_message() {
                                app.status_message = Some(msg);
                            }
                        }
                        asimov_query::GoalContinuation::NoGoal => {
                            app.active_goal_badge = None;
                        }
                    }
                }
...
async fn auth_status(json_output: bool) {
    ...
    let oauth_tokens = if active_provider == "anthropic" {
        asimov_core::oauth::OAuthTokens::load().await
    } else {
        None
    };

    let env_api_key_source = asimov_core::config::api_key_env_vars_for_provider(active_provider)
        .iter()
        .find_map(|env_var| {
            std::env::var(env_var)
                .ok()
    ...
    let stored_api_key_source = provider_status_lookup_keys(active_provider)
        ...
        .find_map(|provider_id| match auth_store.get(provider_id) {
            Some(asimov_core::StoredCredential::ApiKey { key }) if !key.is_empty() => {
    ...
    if json_output {
        ...
        if !logged_in {
            let hint = if active_provider == "anthropic" {
                ...
                asimov_core::config::primary_api_key_env_var_for_provider(active_provider)
            {
...
fn subscription_label(subscription_type: Option<&str>) -> Option<String> {
    match subscription_type? {
        "enterprise" => Some("Asimov Enterprise Account".to_string()),
        "team" => Some("Asimov Team Account".to_string()),
        "max" => Some("Asimov Max Account".to_string()),
        "pro" => Some("Asimov Pro Account".to_string()),
