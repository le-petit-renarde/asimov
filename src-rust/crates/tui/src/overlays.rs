...
pub const ASIMOV_ACCENT: Color = Color::Rgb(233, 30, 99);
pub const ASIMOV_PANEL_BG: Color = Color::Rgb(20, 20, 28);
pub const ASIMOV_PANEL_BORDER: Color = Color::Rgb(72, 72, 80);
pub const ASIMOV_TEXT: Color = Color::Rgb(235, 235, 240);
pub const ASIMOV_MUTED: Color = Color::Rgb(110, 110, 118);
pub const ASIMOV_OVERLAY_BG: Color = Color::Rgb(10, 10, 14);
    ...
    for y in area.y..area.y + area.height {
        for x in area.x..area.x + area.width {
            if let Some(cell) = buf.cell_mut((x, y)) {
                cell.set_bg(ASIMOV_OVERLAY_BG);
                cell.set_fg(ASIMOV_MUTED);
            }
        }
    }
}
...
pub fn render_modal_title_frame(frame: &mut Frame, area: Rect, title: &str, right_hint: &str) {
    ...
    let line = Line::from(vec![
        ...
        Span::styled(" ".repeat(padding), Style::default().fg(ASIMOV_TEXT)),
        Span::styled(
...
pub fn render_help_overlay(frame: &mut Frame, overlay: &HelpOverlay, area: Rect) {
    ...
    use asimov_core::constants::APP_VERSION;
    ...
    let divider_lines: Vec<Line<'static>> = (0..content_area.height)
        .map(|_| Line::from(Span::styled("\u{2502}", Style::default().fg(ASIMOV_MUTED))))
        .collect();
    ...
    for entry in &filtered {
        ...
        right_lines.push(Line::from(vec![
            ...
            Span::styled(aliases_text, Style::default().fg(ASIMOV_MUTED)),
            Span::raw("  "),
            Span::styled(entry.description.clone(), Style::default().fg(ASIMOV_MUTED)),
        ]));
    }
...
// ---------------------------------------------------------------------------
// Pinned-entry persistence  (~/.asimov/history_pins.json)
// ---------------------------------------------------------------------------
...
/// Load the set of pinned entry texts from `~/.asimov/history_pins.json`.
...
/// Persist `pinned_texts` to `~/.asimov/history_pins.json`.
...
impl HistorySearchOverlay {
    ...
    ///
    /// Pinned state is loaded from `~/.asimov/history_pins.json` and applied
    ...
    ///
    /// Persists the updated pin set to `~/.asimov/history_pins.json` and
...
fn kb_line<'a>(key: &str, desc: &str) -> Line<'a> {
    Line::from(vec![
        ...
        Span::styled(desc.to_string(), Style::default().fg(ASIMOV_MUTED)),
    ])
}
...
mod tests {
    ...
    fn modal_search_line_separates_leading_space_from_cursor() {
        let line = modal_search_line("", "Search", ASIMOV_MUTED, ASIMOV_TEXT);
