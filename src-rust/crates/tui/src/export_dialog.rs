...
use crate::overlays::{
    begin_modal_frame, modal_header_line_area, render_modal_title_frame, ASIMOV_ACCENT, ASIMOV_MUTED,
...
fn export_option_row(
    ...
    let fg = if selected { Color::White } else { ASIMOV_TEXT };
    let desc_fg = if selected { Color::Rgb(245, 220, 232) } else { ASIMOV_MUTED };
    let mut spans = vec![
...
pub fn export_as_markdown(
    ...
    if let Some(title) = session_title {
        ...
        out.push_str("# Asimov Conversation Export\n\n");
    }
    for msg in messages {
        let label = match msg.role {
            Role::User => "**User**",
            Role::Assistant => "**Asimov**",
        };
