...
//! Data source: ~/.asimov/stats.jsonl (append-only per-turn usage log)
...
/// A single entry in ~/.asimov/stats.jsonl
...
/// Load and aggregate stats from ~/.asimov/stats.jsonl
pub fn load_stats() -> AggregatedStats {
    let path = dirs::home_dir()
        .map(|h| h.join(".asimov").join("stats.jsonl"))
        .unwrap_or_default();
...
fn tab_span(label: &str, active: bool) -> Span<'static> {
    if active {
        ...
        Span::styled(label.to_string(), Style::default().fg(ASIMOV_MUTED))
    }
}
