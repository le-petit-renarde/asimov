...
impl NamedCommand for AgentsCommand {
    ...
    fn execute_named(&self, args: &[&str], ctx: &CommandContext) -> CommandResult {
        match args.first().copied().unwrap_or("list") {
            "list" => {
                // Load agent definitions from .asimov/agents/ in working dir
                // (and home dir), using the same loader as the TUI agents view.
                let defs = asimov_tui::agents_view::load_agent_definitions(&ctx.working_dir);
            ...
            "create" => {
                ...
                CommandResult::Message(format!(
                    "Create a new agent by adding .asimov/agents/{name}.md\n\
                     Template:\n\
                     ---\n\
                     name: {name}\n\
            ...
            "edit" => {
                ...
                CommandResult::Message(format!(
                    "Edit .asimov/agents/{name}.md in your editor to update the agent."
                ))
            }
            "delete" => {
                let name = match args.get(1).copied() {
                ...
                CommandResult::Message(format!(
                    "Delete .asimov/agents/{name}.md to remove the agent."
                ))
            }
...
impl NamedCommand for AddDirCommand {
    ...
    fn description(&self) -> &str { "Add a directory to Asimov's allowed workspace paths" }
    ...
    fn execute_named(&self, args: &[&str], ctx: &CommandContext) -> CommandResult {
        match args.first().copied().unwrap_or("") {
            "" | "create" => {
                ...
                let result = tokio::task::block_in_place(|| {
                    tokio::runtime::Handle::current().block_on(async move {
                        asimov_core::history::branch_session(
                            &session_id,
                            msg_count,
                            title_opt.as_deref(),
                        )
            ...
            "list" => {
                ...
                let sessions = tokio::task::block_in_place(|| {
                    tokio::runtime::Handle::current()
                        .block_on(asimov_core::history::list_sessions())
                });
            ...
            "switch" => {
                ...
                let result = tokio::task::block_in_place(|| {
                    tokio::runtime::Handle::current()
                        .block_on(asimov_core::history::load_session(&id))
                });
    ...
    fn execute_named(&self, args: &[&str], ctx: &CommandContext) -> CommandResult {
        ...
        match args.first().copied().unwrap_or("list") {
            "list" => {
                let result = tokio::task::block_in_place(|| {
                    tokio::runtime::Handle::current()
                        .block_on(asimov_core::history::load_session(&session_id))
                });
                match result {
                    Ok(session) => {
            ...
            "add" => {
                ...
                let result = tokio::task::block_in_place(|| {
                    tokio::runtime::Handle::current()
                        .block_on(asimov_core::history::tag_session(&session_id, &tag))
                });
            ...
            "remove" => {
                ...
                let result = tokio::task::block_in_place(|| {
                    tokio::runtime::Handle::current()
                        .block_on(asimov_core::history::untag_session(&session_id, &tag))
                });
                    ...
                    Ok(session) => {
                        ...
                        if session.tags.iter().any(|t| t == &tag) {
                            ...
                            let remove_result = tokio::task::block_in_place(|| {
                                tokio::runtime::Handle::current()
                                    .block_on(asimov_core::history::untag_session(&session_id, &tag_clone))
                            });
                            match remove_result {
                                Ok(()) => CommandResult::Message(format!("Removed tag: #{tag}")),
                                Err(e) => CommandResult::Error(format!("Could not remove tag: {e}")),
                            ...
                            let add_result = tokio::task::block_in_place(|| {
                                tokio::runtime::Handle::current()
                                    .block_on(asimov_core::history::tag_session(&session_id, &tag_clone))
                            });
                            match add_result {
                                Ok(()) => CommandResult::Message(format!("Added tag: #{tag}")),
...
impl NamedCommand for PassesCommand {
    ...
    fn description(&self) -> &str { "Share a free week of Asimov with friends" }
    fn usage(&self) -> &str { "claude passes" }

    fn execute_named(&self, _args: &[&str], _ctx: &CommandContext) -> CommandResult {
        CommandResult::Message(
            "Asimov Passes \u{2014} Share Asimov with friends\n\n\
             Share a free week of Asimov with a friend\n\
             Visit https://claude.ai/passes to get your referral link\n\
             Each referral gives your friend 1 week of Asimov Pro"
                .to_string(),
        )
    }
}
...
impl NamedCommand for IdeCommand {
    ...
    fn execute_named(&self, _args: &[&str], _ctx: &CommandContext) -> CommandResult {
        ...
        let env_detection = asimov_core::detect_ide();
        let env_section = match &env_detection {
            Some(kind) => {
                ...
                if let Some(cmd) = kind.extension_install_command() {
                    lines.push(format!("To install the Asimov extension: {}", cmd));
                }
                lines.join("\n")
            }
...
impl NamedCommand for DesktopCommand {
    ...
    fn description(&self) -> &str { "Download and set up Asimov Desktop app" }
    ...
    fn execute_named(&self, _args: &[&str], ctx: &CommandContext) -> CommandResult {
        ...
        // Detect if Asimov Desktop is likely installed (platform-specific heuristic).
        ...
        if let Some(ref session_url) = ctx.remote_session_url {
            ...
            msg.push_str("\u{2713} Already connected to Asimov Desktop\n\n");
            msg.push_str("Your Asimov session is synced with Asimov Desktop.\n\n");
            msg.push_str(&format!("Open this session in Desktop: {deep_link}\n\n"));
            if desktop_likely_installed {
                msg.push_str("Asimov Desktop is installed on this machine.\n");
        ...
        let msg = if os == "macos" {
            if desktop_likely_installed {
                format!(
                    "Open Asimov Desktop \u{2014} macOS\n\n\
                     Asimov Desktop appears to be installed.\n\
                     Launch it from /Applications/Claude.app and sign in with your Anthropic account.\n\n\
                     Download / update: {download_url}"
                )
            } else {
                format!(
                    "Download Asimov Desktop \u{2014} macOS\n\n\
                     Download: {download_url}\n\n\
                     Setup instructions:\n\
                     1. Download and install Asimov Desktop for macOS\n\
                     2. Open Asimov Desktop and sign in with the same Anthropic account\n\
                     3. Asimov will detect the Desktop bridge automatically"
                )
            }
        } else if os == "windows" {
            ...
            if desktop_likely_installed {
                format!(
                    "Open Asimov Desktop \u{2014} Windows{arch_note}\n\n\
                     Asimov Desktop appears to be installed.\n\
                ...
                format!(
                    "Download Asimov Desktop for Windows{arch_note}\n\n\
                     Download: {download_url}\n\n\
                     Setup instructions:\n\
                     1. Download and run the Asimov Desktop installer\n\
                     2. Open Asimov Desktop and sign in with the same Anthropic account\n\
                     3. Asimov will detect the Desktop bridge automatically"
                )
            }
            ...
            format!(
                "Asimov Desktop is not yet available for {os}\n\n\
                 On Linux, you can use Asimov via the CLI or visit https://claude.ai in your browser.\n\
...
impl NamedCommand for MobileCommand {
    ...
    fn description(&self) -> &str { "Download the Asimov mobile app" }
    ...
    fn execute_named(&self, args: &[&str], ctx: &CommandContext) -> CommandResult {
        ...
        out.push_str("Scan to download Asimov mobile app\n");
...
impl NamedCommand for InstallGithubAppCommand {
    ...
    fn description(&self) -> &str { "Set up Asimov GitHub Actions for a repository" }
    fn usage(&self) -> &str { "claude install-github-app" }

    fn execute_named(&self, _args: &[&str], ctx: &CommandContext) -> CommandResult {
        let provider_id = ctx.config.selected_provider_id();
        let provider_secret_step = asimov_core::config::primary_api_key_env_var_for_provider(provider_id)
            .map(|provider_secret| {
                format!(
...
impl NamedCommand for RemoteSetupCommand {
    ...
    fn description(&self) -> &str { "Check and configure a remote Asimov environment" }
    ...
    fn execute_named(&self, _args: &[&str], ctx: &CommandContext) -> CommandResult {
        ...
        let credential_hint = asimov_core::config::api_key_env_vars_for_provider(provider_id);
        ...
        CommandResult::Message(format!(
            ...
            steps.join("\n"),
            if all_ok {
                "\u{2713} All checks passed. Asimov is ready for remote use.\nStart a session: claude --bridge"
            } else {
...
impl NamedCommand for StickersCommand {
    ...
    fn description(&self) -> &str { "Open the Asimov sticker page in your browser" }
