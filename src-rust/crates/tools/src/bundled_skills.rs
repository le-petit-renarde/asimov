...
- User:    ~/.asimov/settings.json
- Project: .asimov/settings.json
- Local:   .asimov/settings.local.json
...
6. Report: what was verified, what passed, what failed or is uncertain."#,
    ...
    BundledSkill {
        name: "update-config",
        description: "Configure Asimov settings (hooks, permissions, env vars, behaviours) via settings.json.",
...
Modify Asimov configuration by updating settings.json files.

## Settings File Locations

| File | Scope | Use For |
|------|-------|---------|
| `~/.asimov/settings.json` | Global | Personal preferences for all projects |
| `.asimov/settings.json` | Project | Team-wide hooks, permissions, plugins |
| `.asimov/settings.local.json` | Project (local) | Personal overrides for this project |
