...
impl PluginRegistry {
    ...
    pub fn all_mcp_servers(&self) -> Vec<asimov_core::config::McpServerConfig> {
        let mut servers: Vec<asimov_core::config::McpServerConfig> = Vec::new();
        for plugin in self.enabled() {
            for mcp in &plugin.manifest.mcp_servers {
                servers.push(asimov_core::config::McpServerConfig {
                    name: mcp.name.clone(),
