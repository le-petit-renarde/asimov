// asimov-plugins: Plugin runtime for the Asimov CLI.
//
...
///
/// Search order:
/// 1. `~/.asimov/plugins/`  (user-global)
/// 2. `<project_dir>/.asimov/plugins/`  (project-local)
...
///
/// Copies the plugin directory into `~/.asimov/plugins/` and returns the
...
mod tests {
    ...
    async fn load_plugins_finds_project_plugin() {
        let tmp = TempDir::new().unwrap();
        let plugin_dir = tmp.path().join(".asimov").join("plugins").join("test-plugin");
