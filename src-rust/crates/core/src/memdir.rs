...
/// 2. `<ASIMOV_REMOTE_MEMORY_DIR>/projects/<sanitized-root>/memory/`
///    when `ASIMOV_REMOTE_MEMORY_DIR` is set.
/// 3. `~/.asimov/projects/<sanitized-root>/memory/` (default).
pub fn auto_memory_path(project_root: &Path) -> PathBuf {
    ...
    let memory_base = std::env::var("ASIMOV_REMOTE_MEMORY_DIR")
        .map(PathBuf::from)
        .unwrap_or_else(|_| {
            dirs::home_dir()
...
/// 1. `ASIMOV_DISABLE_AUTO_MEMORY` — truthy → OFF, falsy (but defined) → ON.
/// 2. `ASIMOV_SIMPLE` (--bare) → OFF.
/// 3. Remote mode without `ASIMOV_REMOTE_MEMORY_DIR` → OFF.
...
pub fn is_auto_memory_enabled(settings_enabled: Option<bool>) -> bool {
    if let Ok(val) = std::env::var("ASIMOV_DISABLE_AUTO_MEMORY") {
    ...
    if std::env::var("ASIMOV_SIMPLE").is_ok() {
        return false;
    }

    if std::env::var("ASIMOV_REMOTE").is_ok()
        && std::env::var("ASIMOV_REMOTE_MEMORY_DIR").is_err()
    {
        return false;
    }
