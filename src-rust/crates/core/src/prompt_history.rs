...
//   - Entries are written to `~/.asimov/history.jsonl` via O_APPEND.
...
//     `~/.asimov/pastes/<sha256-hex>` and referenced by hash.
...
/// Persist `text` to `~/.asimov/pastes/<hash>`.  Fire-and-forget.
...
/// Read text from `~/.asimov/pastes/<hash>`, returning `None` if missing.
...
/// Append `entry` to `~/.asimov/history.jsonl`.
///
/// The call is fire-and-forget: it spawns a background Tokio task and returns
/// immediately.  If `ASIMOV_SKIP_PROMPT_HISTORY` is truthy the call is
/// a no-op.
pub fn add_to_history(entry: HistoryEntry) {
    if std::env::var("ASIMOV_SKIP_PROMPT_HISTORY")
...
/// Read `~/.asimov/history.jsonl`, filter by `project`, and return up to
