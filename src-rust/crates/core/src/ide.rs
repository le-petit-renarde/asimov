...
impl IdeKind {
    ...
    /// Install command for the Asimov extension (if known for this IDE).
    pub fn extension_install_command(&self) -> Option<String> {
        match self {
            Self::VSCode => Some("code --install-extension asimov.asimov".to_string()),
            Self::VSCodeInsiders => {
                Some("code-insiders --install-extension asimov.asimov".to_string())
            }
            Self::Cursor => Some("cursor --install-extension asimov.asimov".to_string()),
            Self::Windsurf => Some("windsurf --install-extension asimov.asimov".to_string()),
            Self::VSCodium => Some("codium --install-extension asimov.asimov".to_string()),
            _ => None,
        }
    }
}
