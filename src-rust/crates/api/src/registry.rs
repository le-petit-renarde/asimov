...
impl ProviderRegistry {
    ...
    /// `~/.asimov/codex_tokens.json`.  Returns `&mut self` for builder chaining.
    ...
    /// persistent [`AuthStore`] (`~/.asimov/auth.json`) for credentials.
    ///
    /// This ensures that API keys stored via `/connect` or `asimov auth` are
    ...
    ///
    /// [`AuthStore`]: asimov_core::AuthStore
    pub fn from_environment_with_auth_store(anthropic_config: ClientConfig) -> Self {
        ...
        for (provider_id, _cred) in &auth_store.credentials {
            let pid = asimov_core::ProviderId::new(provider_id.as_str());
